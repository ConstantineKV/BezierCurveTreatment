...
double BezierCurve::polinome(int i, unsigned long n, double t)
{
...
bool BezierCurve::CheckBody(string body, string argument, size_t number_of_points)
{
    vector<string> summands;
    size_t pos1 = 0, pos2 = 0, position = 0;
    ...
    for (unsigned int i = 0; i < summands.size(); i++)
    {
        if (i == 0)                                         //first summand. Example: (1-t)^3
        {
            ...
            else
            {
                ...
                compare_str.append(argument);
                compare_str.append(")^");
                compare_str.append(IntToString(static_cast<int>(number_of_points - 1)));
            }
        ...
        if ((i > 0) && (i < number_of_points - 1))                              //inner summand. Example: 3*t*(1-t)^2*P1 or 3*t^2*(1-t)*P2
        {
            ...
            pos1 = summands[i].find('*', pos1);
            temp_str = summands[i].substr(0, pos1);
            compare_str = IntToString(binome(i, static_cast<int>(number_of_points - 1)));
            if ((temp_str != compare_str) || (pos1 == -1))
                return false;
            //check argument t
            pos1++;
            ...
            else
            {
                ...
                compare_str.append(argument);
                compare_str.append(")^");
                compare_str.append(IntToString(static_cast<int>(number_of_points - 1 - i)));
            }
            if ((temp_str != compare_str) || (pos2 == -1))
                return false;
            pos1 = pos2;
...
bool BezierCurve::CheckDescription(string description)
{
    ...
    size_t number_of_points = 0, pos1 = 0, pos2 = 0;
...
{
    size_t position = Description.find('(', 0);
    return Description.substr(0, position);
}

string BezierCurve::NameFromDescription(string Description, size_t& position)
{
...
string BezierCurve::ArgumentFromDescription(string Description)
{
    ...
    size_t pos2 = Description.find(')', pos1);
    return Description.substr(pos1 + 1, pos2 - pos1 - 1);
}

string BezierCurve::ArgumentFromDescription(string Description, size_t& position)
{
    size_t pos1 = Description.find('(', position);
    size_t pos2 = Description.find(')', pos1);
    position = pos2;
    return Description.substr(pos1 + 1, pos2 - pos1 - 1);
...
string BezierCurve::BodyFromDescription(string Description, size_t& position)
{
    size_t pos1 = Description.find('=', position);
    size_t pos2 = Description.find(' ', pos1);
    position = pos2;
...
size_t BezierCurve::NumberOfPointsFromDescription(string Description)
{
...
{
    point_b Dot;
    size_t pos1 = point_string.find('(', 0);
    size_t pos2 = point_string.find(';', pos1);
    string x = point_string.substr(pos1 + 1, pos2 - pos1 - 1);
    pos1 = point_string.find(';', pos2);
    pos2 = point_string.find(';', pos1 + 1);
    string y = point_string.substr(pos1 + 1, pos2 - pos1 - 1);
...
point_b BezierCurve::ÑalculatePoint(double t)
{
    ...
    if (Is_Ready())
    {
        unsigned long n = NumberOfPoints - 1;
        for (int i = 0; i <= n; i++)
        {
...
int BezierCurve::MinDistanceFull(BezierCurve* SecondCurve, double& Distance, double& FirstArg, 
    double& SecondArg, double coefficient, double tolerance)
{
    int error_b;
